...
double kmeans(int n, int K, 
  ...
  int try;
  for (try = 0; try < 5; try++) {
    double dl = kmeans1(n, K, p, membership[1], clusters[1], try);
  ...
  return log(R) * dim * K + n * log(K) + min_dl;
}

double kmeans_parallel(int n, int K, point * p,
                       int ** membership,
                       cluster ** clusters) {
  return 0.0;
}
...
int main(int argc, char ** argv) {
  ...
  long i;
  for (i = 0; i < n; i++) {
    int rs = fscanf(fp, "%lf %lf\n", &p[i].p[0], &p[i].p[1]);
    assert(rs == 2);
  }
  fclose(fp);

  int * membership[3] = { malloc(sizeof(int) * n), 
			  malloc(sizeof(int) * n),
			  malloc(sizeof(int) * n) };
  ...
  for (K = max_K; K >= 2; K = (int)(K * 2.0/3.0)) {
    ...
    double dl = kmeans(n, K, p, membership+1, clusters+1);
    //double dl = kmeans_parallel(n, K, p, membership+1, clusters+1);
  ...
  dump_plot(n, min_K, p, membership[0], clusters[0], wp);
  fclose(wp);
  for (i = 0; i < 3; i++) {
    free(membership[i]);
    free(clusters[i]);
